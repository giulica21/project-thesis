...
\end{proof} 
\begin{remark}
    For $\alpha=1$ we define the inverse of the radius fo convergence as the \textit{the connective constant}, such that $R_{\chi^D}^{-1} \eqcolon \mu_d$.\\ We can find a slightly better estimate for $\mu_d$, which is $\mu_d \le 2d-1$ (see Claim \ref{claim:mud} ) instead of $\mu_d \le 2d$ (as proven before in Claim \ref{claim:radius}). The $2d -1$ comes from the property of the SAW of not jumping back to the site we came from.
\end{remark}
...
\begin{equation}
    G^\Lambda(x, y) = G^\Lambda(x, x) \sum_{x_1 \in \Lambda \setminus \{x\}} \triangle^\alpha(x, x_1) G^{\Lambda \setminus \{x\}}(x_1, y)
\end{equation}
\end{claim}
...
\]
For $T_{x_1,x_2} \ne 0$, we need $x_1 = x$ and $x_2 \ne x$.

For the rightmost matrix element to be non-zero when $y \neq x$, we strictly need $x_2 \neq x$ (since the decoupled resolvent cannot transition between $x$ and the rest of the lattice). For the perturbation term $T_{x_1, x_2}$ to be non-zero under the condition $x_2 \neq x$, we must have $x_1 = x$. 

Applying these constraints, the corresponding matrix element of the perturbation is exactly $-T_{x, x_2} = -(-\Delta)^\alpha(x, x_2)$. Following standard notation for the fractional Anderson model, we define this strictly positive jump weight as $\triangle^\alpha(x, x_2) \coloneqq -(-\Delta)^\alpha(x, x_2)$. Substituting this back into the sum leaves us with:
\[
G^\Lambda(x, y) = \sum_{x_1 \in \Lambda \setminus \{x\}} G^\Lambda(x, x) \triangle^\alpha(x, x_1) G^{\Lambda \setminus \{x\}}(x_1, y).
\]
\end{proof}
