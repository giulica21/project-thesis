...
\textit{ fractional moment method} (FMM) by Aizenman and Molchanov \cite{aizenman1993}. The multiscale analysis method focuses on the bound of the second moments of the operator's resolvent, whereas the fractional moments method utilizes its fractional powers. In this thesis, we are going to focus on the second one, since it provides a simple
...
In Chapter 1, we introduce the Anderson model, focusing on the properties of its components, its spectrum, and the main results known to date.\\
...
\noindent
In Chapter 4, we provide the preliminary definitions for self-avoiding walk (SAW) methods and obtain the main results of this thesis using a new representation for the Green function.\\

\noindent
Finally,  Chapter 5 offers a comparison between the standard model with nearest-neighbor jumps and a model with long-range jumps. We define the fractional discrete Laplacian and basically follow the proof structure of the previous Chapters in order to obtain similar results but with a polynomial decay.
\subsection*{Prerequisites}
It is useful to have some familiarity with measure theory, basic probabilistic concepts, foundations of the theory of linear operators in Hilbert spaces and the consequences of the spectral theorem for self-adjoint operators. \\


\clearpage
\chapter{The Anderson Model}
In this chapter, the random Schr\"odinger operator is the center of our attention. First, we define it as the \textit{Hamiltonian} and discuss the physical meaning of its time-evolving solutions. Right after, we focus on the properties of each of its components and their spectra. Finally, we discuss the main results and some open problems in the field.\\
...
\end{equation}
where $-\Delta$ is a deterministic matrix representing the kinetic energy, $V_\omega$ is a diagonal matrix with random entries representing the disordered potential, and $\lambda$ is the disorder parameter.
...
As we will see later, under certain conditions on the disorder parameter, this probability $P$ tends to $0$ faster than any polynomial at all times $t$ as the distance $|x-y| \to \infty$, mathematically proving that the particle remains trapped.
\vspace{0.3 cm}

\subsection*{The  Discrete Laplacian} 
The  discrete Laplacian is the  discrete analogue of the usual Laplacian ($ \sum_j \partial^2/\partial x_j^2$).
...
The operator $-\Delta$ is bounded and self-adjoint on $\ell^2(\mathbb{Z}^d)$. To establish boundedness, we verify that $-\Delta u \in \ell^2(\mathbb{Z}^d)$ for any $u \in \ell^2(\mathbb{Z}^d)$. We can show this through the following,
\begin{align*}
...
This guarantees the operator is bounded:
\[
\frac{\|-\Delta u\|_{\ell^2}^2}{\|u\|_{\ell^2}^2} \le 16 d^2 \implies \|-\Delta\| = \sup_{u \ne 0} \frac{\|-\Delta u\|_{\ell^2}}{\|u\|_{\ell^2}} \le 4d.
\]

To prove self-adjointness for a bounded operator, we only need to show symmetry. For $u, v \in \ell^2(\mathbb{Z}^d)$:
\begin{align*}
...
The next goal is to determine the spectrum of $H_\omega$. Because the random potential entries $(\omega_x)_{x\in\mathbb{Z}^d}$ are independent and identically distributed, the random Hamiltonian $H_\omega = -\Delta + \lambda V_\omega$ forms an ergodic family of bounded self-adjoint operators. A standard consequence of ergodic operator theory is that the spectrum of $H_\omega$ is almost surely deterministic: there exists a closed, non-random set $\Sigma \subset \mathbb{R}$ such that
\begin{equation}
...
\end{equation}
Recall from Section~\ref{sec:The_Hamiltonian} that the spectrum of the discrete Laplacian is the band $\sigma(-\Delta) = [0, 4d]$. Since the random potential $V_\omega$ has i.i.d. entries distributed uniformly on $[-1, 1]$, the almost sure spectrum of $\lambda V_\omega$ is the interval $[-\lambda, \lambda]$. Consequently, the almost sure spectrum of $H_\omega$ is given by the compact interval:
\begin{equation}
...
\begin{definition}\label{def:spectralmeasuredef}
    ...
    \begin{equation*}
        \langle \phi, f(A)\phi \rangle = \int_{\sigma(A)} f(\xi) \, d\mu_{\phi}(\xi).
    \end{equation*}
...
In terms of this classification, Anderson localization corresponds exactly to the absence of continuous spectrum, ensuring that $\sigma(H_\omega) = \sigma_{\mathrm{pp}}(H_\omega)$ almost surely.
...
\begin{equation}
    \mathbb{E}\left( |(H_\omega - z)^{-1}(x,y)|^s \right) \le C e^{-\mu |x-y|}
\end{equation}
uniformly for $z \in \mathbb{C} \setminus \mathbb{R}$ and all $x,y \in \mathbb{Z}^d$, with $C < \infty$ and $\mu > 0$. Here, $\lambda_0 = \lambda_{\text{And}}$ is a fixed threshold proposed by P.W. Anderson (\cite{anderson1958}), which was rigorously proven later by J. Schenker (\cite{schenker2015}) using self-avoiding walks. \\

Naturally, the resolvent is well-defined only when $z \notin \sigma(H_\omega)$. The \textit{fractional moments method} was revolutionary because averaging a fractional power of the resolvent avoids these divergences, allowing the bound to hold uniformly for all $z \in \mathbb{C} \setminus \mathbb{R}$. \\

Originally, the absence of conductivity was proven using integer powers instead of fractional ones. This creates divergence problems near zero and requires complicated analytical tools like the MSA method. \\
...
We understood what localization means from an heuristic point of view. In the previous Chapter, it came out as a spectral property of the Hemiltonian. In this section, we also introduce dynamical properties for localization, together with the implications between them. The definitions follow the ones given by A. Klein in \cite{klein2007}.
\section{Spectral localization}


\begin{definition}[Spectral localization]
We say that $H = H_\omega$ exhibits \textbf{spectral localization} if it has pure point spectrum almost surely. That is
\begin{equation}
...
\begin{definition}[Dynamical localization]
    We say that $H_\omega$ shows dynamical localization if for all $p>0$ and $\phi \in \ell^2(\mathbb{Z}^d)$,
    \begin{equation}
    \sup_{t \in \mathbb{R}} \left\| |X|^p\,e^{-it H_\omega}\phi \right\| < \infty \label{eq:dynloc}
    \end{equation}
...
\begin{definition}[Exponential localization]
    We say that $H_\omega$ shows exponential localization if 
    \begin{equation}
        \mathbb{E}\left( \sup_{t \in \mathbb{R}} |e^{-itH_\omega}(0,x) |\right) \leq C e^{- \mu |x|}\label{eq:explocalisation}
    \end{equation}
for $x \in \mathbb{Z}^d$
\end{definition}
Furthermore the eigenfunctions corresponding to all eigenvalues $E$ in the almost sure spectrum, decay exponentially: for almost all $\omega$, $H_\omega$ has a complete set of eigenfunctions $\phi_E(., \omega)$ satisfying
$$|\phi_E(x, \omega)| \le C_{\omega}e^{-\mu|x-x_E(\omega)|}$$
where $\mu > 0$ is the localization length parameter, $x_E(\omega)$ are the centers of localization, and $C_{\omega  } < \infty$ is a finite constant.
\begin{remark}
Dynamical localization implies that all moments of the distance between the two sites are bounded uniformly in time (recall that we are working in an infinite grid, and with translation invariance, so as well as $(0,x)$ we can consider any site in the grid).
...
\begin{proof}
To see how (2.1) follows from (2.2), we assume that $\psi(k) = 0$ for $|k| > R$, since $\psi$ has compact support.  Then
\begin{align*}
    \||X|^p e^{-itH_\omega} \psi\|^2 &= \sum_j |\langle \delta_j, |X|^p e^{-itH_\omega} \psi \rangle|^2\\
    &= \sum_j |j|^{2p} |\sum_{|k|\le R} e^{-itH_\omega} (j,k) \psi(k)|^2\\
    &\le \sum_j \sum_{|k|\le R} |j|^{2p} | e^{-itH_\omega} (j,k) |^2 ||\psi||^2.
\end{align*}
where the last step used the Cauchy-Schwarz inequality.\\
Since $|e^{-itH_\omega} (j,k)|^2 \le 1$:
\begin{align*}
    \mathbb{E}(\sup_t |||X|^p e^{-itH_\omega} \psi||^2) &\le \sum_j \sum_{|k|\le R} |j|^{2p}\,\, \mathbb{E}(\sup_t |e^{-itH_\omega} (j,k)|) ||\psi||^2\\ 
...
\begin{remark}
     Dynamical localisation holds \textit{almost surely} rather than for every realisation, because our bounds control only the \textit{expected} value over the disorder. A bound on the average does not exclude that a single realisation is large. However, if the expectations decay fast enough, then being large at distance $|x|$ is a very unlikely event for each $x$, and the probabilities of these events are summable. The Borel--Cantelli lemma below then shows that, with probability one, this can happen only for finitely many $x$.
\end{remark}

\begin{lemma}[Borel--Cantelli]\label{lemma:almostsurely}
Let $(\Omega,\mathcal{F},\mathbb{P})$ a probability space. Let $n \rightarrow A_n \in \mathcal{F}$ a sequence of events. 
Then if $\sum_{n \in \mathbb{N}}(A_n) < \infty$, almost surely, only finitely many of the $A_n$ occur, which means $\mathbb{P}(\limsup_{n \to \infty} An)=0$
\end{lemma}


\begin{example}[From proposition \ref{prop:exptodyn}]
Set $X_x := \sup_{t\in\mathbb{R}} \bigl|e^{-itH_\omega}(0,x)\bigr|$ and consider the events
\begin{equation*}
  A_x := \bigl\{ X_x > e^{-\mu|x|/2} \bigr\}.
\end{equation*}
By Markov's inequality and \eqref{eq:explocalisation},
\begin{equation*}
\mathbb{P}(A_x) \le \frac{\mathbb{E}[X_x]}{e^{-\mu|x|/2}} \le C e^{-\mu|x|/2}.
\end{equation*}
This is summable over $x \in \mathbb{Z}^d$, so $\sum_x \mathbb{P}(A_x) < \infty$.
By Lemma \ref{lemma:almostsurely}, almost surely only finitely many $A_x$ occur.
Hence there is a random radius $R(\omega) < \infty$ such that
$X_x \le e^{-\mu|x|/2}$ for all $|x| > R(\omega)$.

Split the sum at $R(\omega)$, considering $X_x \le 1$:
\begin{equation*}
\sum_j \sum_{|k|\le R} |j|^{2p} | e^{-itH_\omega} (j,k) |^2 ||\psi||^2
\le \sum_{|j|\le R(\omega)} |j|^{2p}||\psi||^2 + \sum_{|j|>R(\omega)}\sum_{|k|\le R} |j|^p e^{-\mu|j-k|}||\psi||^2 < \infty .
\end{equation*}
for almost every realization of $\omega \in \Omega$.   
\end{example}
...
We say that $H = H_\omega$ exhibits \textbf{spectral localization} if it has pure point spectrum almost surely. That is,
\begin{equation}
    \sigma(H_\omega) = \sigma_{\mathrm{pp}}(H_\omega) \quad \text{and} \quad \sigma_{\mathrm{c}}(H_\omega) = \emptyset    
\end{equation}
for almost every realization of $\omega \in \Omega$.
\end{definition}
...
\begin{definition}[Dynamical localization]
    We say that $H_\omega$ exhibits dynamical localization if for all $p>0$ and $\phi \in \ell^2(\mathbb{Z}^d)$ of finite support,
    \begin{equation}
    \sup_{t \in \mathbb{R}} \left\| |X|^p\,e^{-it H_\omega}\phi \right\| < \infty \label{eq:dynloc}
    \end{equation}
    for almost every realization of $\omega \in \Omega$, where the position operator $|X|$ is defined by $(|X|\phi)(n) = |n|\phi(n)$.
...
\begin{definition}[Exponential localization]
    We say that $H_\omega$ exhibits exponential localization if 
    \begin{equation}
...
Furthermore, the eigenfunctions corresponding to all eigenvalues $E$ in the almost sure spectrum decay exponentially: for almost every $\omega$, $H_\omega$ has a complete set of eigenfunctions $\phi_E(\cdot, \omega)$ satisfying
$$|\phi_E(x, \omega)| \le C_{\omega}e^{-\mu|x-x_E(\omega)|}$$
where $\mu > 0$ is the localization length parameter, $x_E(\omega)$ are the centers of localization, and $C_{\omega} < \infty$ is a finite constant.
...
\begin{proof}
To see how \eqref{eq:dynloc} follows from \eqref{eq:explocalisation}, assume that $\psi(k) = 0$ for $|k| > R$ (i.e., $\psi$ has finite support). Then
\begin{align*}
    \||X|^p e^{-itH_\omega} \psi\|^2 &= \sum_j |\langle \delta_j, |X|^p e^{-itH_\omega} \psi \rangle|^2\\
    &= \sum_j |j|^{2p} \left|\sum_{|k|\le R} \langle \delta_j, e^{-itH_\omega} \delta_k \rangle \psi(k)\right|^2\\
    &\le \sum_j \sum_{|k|\le R} |j|^{2p} |\langle \delta_j, e^{-itH_\omega} \delta_k \rangle|^2 \|\psi\|^2,
\end{align*}
where the last step applies the Cauchy-Schwarz inequality.

Because $|\langle \delta_j, e^{-itH_\omega} \delta_k \rangle| \le 1$, it follows
\begin{align}
    \mathbb{E}\left(\sup_{t \in \mathbb{R}} \||X|^p e^{-itH_\omega} \psi\|^2\right) &\le \sum_j \sum_{|k|\le R} |j|^{2p}\,\, \mathbb{E}\left(\sup_{t \in \mathbb{R}} |\langle \delta_j, e^{-itH_\omega} \delta_k \rangle|\right) \|\psi\|^2\\ \nonumber
    &\le C \sum_j \sum_{|k|\le R} |j|^{2p} e^{-\mu|j-k|} \|\psi\|^2 \\
    &< \infty.\nonumber
\end{align}
\end{proof}
...
\begin{remark}
    Dynamical localization holds \textit{almost surely} rather than for every single realization because our bounds control only the \textit{expected} value over the disorder. A bound on the average does not strictly exclude the possibility that a specific realization is large. However, if the expectations decay fast enough, then being large at a great distance $|x|$ is a very unlikely event. The probabilities of these events are summable, and the Borel--Cantelli lemma below guarantees that, with probability one, this can happen only for finitely many $x$.
\end{remark}

\begin{lemma}[Borel--Cantelli]\label{lemma:almostsurely}
Let $(\Omega,\mathcal{F},\mathbb{P})$ be a probability space, and let $(A_n)_{n \in \mathbb{N}}$ be a sequence of events in $\mathcal{F}$. 
If $\sum_{n \in \mathbb{N}} \mathbb{P}(A_n) < \infty$, then almost surely, only finitely many of the events $A_n$ occur. That is, $\mathbb{P}(\limsup_{n \to \infty} A_n)=0$.
\end{lemma}

\begin{example}[From Proposition \ref{prop:exptodyn}]
Set $X_x := \sup_{t\in\mathbb{R}} \bigl|\langle \delta_0, e^{-itH_\omega} \delta_x \rangle\bigr|$ and consider the events
\begin{equation*}
  A_x := \bigl\{ X_x > e^{-\mu|x|/2} \bigr\}.
\end{equation*}
By Markov's inequality and \eqref{eq:explocalisation},
...
\mathbb{P}(A_x) \le \frac{\mathbb{E}[X_x]}{e^{-\mu|x|/2}} \le C e^{-\mu|x|/2}.
\end{equation*}
This is summable over $x \in \mathbb{Z}^d$, meaning $\sum_x \mathbb{P}(A_x) < \infty$.

By Lemma \ref{lemma:almostsurely}, almost surely only finitely many $A_x$ occur. Hence, there exists a random, finite radius $R(\omega) < \infty$ such that $X_x \le e^{-\mu|x|/2}$ for all $|x| > R(\omega)$.

We can then split the sum at $R(\omega)$ and use the trivial bound $X_x \le 1$ for the interior terms:
\begin{equation*}
\sum_j \sum_{|k|\le R} |j|^{2p} X_{j-k}^2 \|\psi\|^2
\le \sum_{|j|\le R(\omega)} \sum_{|k|\le R} |j|^{2p}\|\psi\|^2 + \sum_{|j|>R(\omega)}\sum_{|k|\le R} |j|^{2p} e^{-\mu|j-k|}\|\psi\|^2 < \infty
\end{equation*}
for almost every realization of $\omega \in \Omega$.   
\end{example}
...
\begin{remark}
    We distinguish \eqref{eq:FMM} from \eqref{eq:schenkerbound} in this theorem because the first represents a generalized bound for any absolutely continuous distribution of $\omega$, while the second considers the specific case of a uniform distribution on $[-1,1]$, yielding a more precise value for $\lambda_0$. Both are equally valid for proving localization, but our ultimate goal is to reach \eqref{eq:schenkerbound}. 
\end{remark}

Before achieving this result, we introduce two preparatory lemmas: the so-called \textit{a priori bound} on fractional moments and the \textit{resolvent identity}.

\begin{lemma}[A priori bound] \label{lemma:aprioribound}
Let $s \in (0,1)$. Then it holds
\begin{equation}
\mathbb{E}_{x}(|G_{z}(x, x)|^s) \le \frac{1}{(1-s) \,\lambda^{s}}
\end{equation}
...
This proof demonstrates the simplicity of the fundamental idea underlying the FMM . For a fixed $x \in \mathbb{Z}^d$, we write $\omega = (\hat{\omega}, \omega_x)$, where $\hat{\omega}$ is shorthand for the variables at all other sites, $(\omega_n)_{n \in \mathbb{Z}^d \setminus \{x\}}$ . Using $P_{e_x} := \langle e_x, \cdot \rangle e_x$ to denote the orthogonal projection onto the span of the basis vector $e_x$, we can separate the dependence of $H_{\omega}$ on $\omega_x$ and $\hat{\omega}$ as follows :
\[
...
\begin{equation}
G_{z}(x, x) = \hat{G}_{z}(x, x) - \lambda \omega_x \hat{G}_{z}(x, x) G_{z}(x, x),
\end{equation}
where $\hat{G}_{z}$ denotes the Green function of $H_{\hat{\omega}}$ . Solving for $G_{z}(x, x)$, we obtain
\begin{equation}
G_{z}(x, x) = \frac{1}{a + \lambda \omega_x} \quad \text{with } a = \frac{1}{\hat{G}_{z}(x, x)}.
\end{equation}
Note that this expression is well-defined because one can easily check through the resolvent identity $\mathrm{Im} \hat{G}_{z}(x, x) > 0$ whenever $\mathrm{Im} z > 0$ .
...
\begin{equation}
\mathbb{E}_x(|G_{z}(x, x)|^s) = \frac{1}{2 \lambda^s} \int_{-1}^{1} \frac{d\omega_x}{\left|\frac{a}{\lambda} + \omega_x\right|^s} \le \frac{1}{2 \lambda^s} \int_{-1}^{1} \frac{d\omega_x}{|\omega_x|^s} = \frac{1}{(1-s)\lambda^s}.
\end{equation}
...
\subsection{From fractional moments to dynamical localization}
...
\begin{theorem}\label{thm:theorem2}
    ...
    \end{equation}
    uniformly in $E \in \mathbb{R}$ and $\eta > 0$, then exponential localization holds.
\end{theorem}
...
From \eqref{eq:explocalisation} we can also directly obtain spectral localization through Simon-Wolff criterion.
...
\begin{definition}
    Let $ -\Delta$ be defined as in Section \ref{sec:The_Hamiltonian}. The fractional  discrete Laplacian is $ (-\Delta)^\alpha$ for $ 0< \alpha<1$
\end{definition}
Just as the standard  discrete Laplacian, $ (-\Delta)^\alpha$ is bounded, self-adjoint and translation invariant. Its entries satisfy $$ (-\Delta)^\alpha(x,x) >0 \quad \forall x \in \mathbb{Z}^d, \qquad  (-\Delta)^\alpha(x,y) < 0 \quad \forall x \ne y \,\,\text{in}\,\, \mathbb{Z}^d$$
