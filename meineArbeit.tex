...
This is in sharp contrast to the behavior of electrons in ideal, perfectly ordered crystals, which naturally act as conductors.\\
...
\textit{ fractional moment method} (FMM) by Aizenman and Molchanov \cite{aizenman1993}. The multiscale analysis method focuses on the bound of the second moments of the operator's resolvent, whereas the fractional moments method utilizes its fractional powers. In this thesis, we are going to focus on the second one, combined with the self-avoiding-walks method, like it is doone by J.Schenker in \cite{schenker2015}.\\

Studies in this field also differentiate between two variations of this model. The standard model considers quantum jumps of distance one, while the fractional model introduces jumps of arbitrarily long distances in the grid, as shown by M. Disertori, R. Escobar, and C. Rojas-Molina in \cite{disertori2024}. This change impacts the localization criteria, which are weaker for the fractional model. Because the proof structures are highly similar, this thesis considers a generalized operator that represents both types. Whenever differences arise, they will be explicitly explained in the text.\\
...
As we will see later, under certain conditions on the disorder parameter, this probability $P$ tends to $0$ at all times $t$ as the distance $|x-y| \to \infty$, mathematically proving that the particle remains trapped.
\vspace{0.3 cm}

\subsection*{The  Discrete Laplacian}
The  discrete Laplacian is the  discrete analogue of the usual Laplacian ($ \sum_j \partial^2/\partial x_j^2$). To simplify calculations and highligth the main differences, first we focus on the case $\alpha=1$ and later we compare it with $\alpha \in (0,1)$.
...
The intuition behind the fractional method is  and can be understood from the following integral, which we will encounter in upcoming proofs (for example, see Lemma \ref{lemma:aprioribound}):
\[
...
\begin{align*}
    ...
    &= \sum_{x \in \mathbb{Z}^d} |x|^{2p} \left|\sum_{|k|\le R}  e^{-itH_{\omega,\alpha}}(k,x) \psi(k)\right|^2\\
\end{align*}
Assume that the particle is intially localized in the origin (\textit{recall: we work under translation invariance}). Then we obtain $$ = \sum_{x \in \mathbb{Z}^d} |x|^{2p} \left|\sum_{|k|\le R}  e^{-itH_{\omega,\alpha}}(k,x) \delta_0 \right|^2 =\sum_{x \in \mathbb{Z}^d} |x|^{2p} | e^{-itH_{\omega,\alpha}}(0,x)|^2$$
\end{proof}
...
In this section, we introduce the key lemmas and theorems that help us prove localization. The goal of this section is to demonstrate the power of the fractional moments method (FMM). First, we define the Green function, and then we prove the first fractional bound: the \textit{a priori bound}. This will provide a good example of how we use  but important analytic tools, such as the \textit{resolvent identity}. Finally, we will prove that localization holds on some conditions on the disorder parameter, using the bound on the fractional power of the Green function. Most of the proof come from \cite{stolz2011},\cite{aizenman2015} and \cite{disertori2024}.
...
The following lemmas are the most important tools in the main proofs of this thesis.
...
\end{proof}
From the resolvent identity we can obtain a new representation of the Green function, which will help us develop the successive proofs.
...
The following theorems are the goal of this thesis. However, not of this Chapter. Here we are going to use their results to check which type of localization follow. The whole proof of Theorem \ref{thm:theorem1.1} and Theorem \ref{thm:theorem1.2} need  both FMM and SAW, so they will be given in Chapter \ref{sec:SAW}.
...
\end{equation}
uniformly in $z \in \mathbb{C}\setminus \mathbb{R}$ with $C < \infty$ and $\mu >0$, where $\lambda_0(s)$ is the unique solution of $\lambda_0= \mu_d \, e\,\ln(\lambda_0)$.
\end{theorem}
...
\end{theorem}
\begin{remark}
    $\mu_d $ and $R_{\chi^D}$ are defined with self-avoiding walks in Chapter \ref{sec:SAW}.
\end{remark}
...
\begin{theorem}\label{thm:simonwolfftheorem}
    Let Theorem \ref{thm:theorem1.1} and \ref{thm:theorem1.2} and  hold. Then, on those conditions, the spectrum of $H_{\omega,\alpha}$ consists only of pure point spectrum for a.e. realisation of $V_\omega$.
\end{theorem}
\begin{proof}[Proof from \cite{disertori2024}]
    ...
    Moreover, from \cite[Proposition 5.1]{stolz2011} we also notice that  $$\sum_{y \in \mathbb{Z}^d} \mathbb{E}[|G_{E+i\eta,\alpha}(x,y)|^2] \le \frac{C}{\eta} \sum_{y \in \mathbb{Z}^d} \mathbb{E}[|G_{E+i\eta,\alpha}(x,y)|^2]$$ which deverges when $\eta \rightarrow 0$.\\
    We argue:
    \begin{align*}
...
\begin{proposition}\label{prop:almostloc}
    ...
    \begin{equation}
        \mathbb{E}[Q(x,y;\Sigma)] \le C_1 \lim_{|\eta| \downarrow 0} \int_\Sigma \mathbb{E}[|G(x,y;E+i\eta)|^s] \, dE
    \end{equation}
    with a finite constant $C_1 \coloneq C_1(s,\rho) < \infty$
\end{proposition}
...
\begin{proof}[Proof of Proposition \ref{prop:almostloc}]
    ...
    \end{align*}
    where we used $ |\sum _n a_n|^s \le \sum_n |a_n|^s$ in the last step.\\
    Now, we average $I$ in $\omega_x$, in order to obtain the following bound:
    \begin{align*}
    \mathbb{E}_x [I] &\le \mathbb{E}_x [L] \\
    &= (|\zeta|^s - |\lambda a|^s) \cdot |G_{E+i\eta}^{(\zeta)}(x,y)|^s \cdot \frac{1}{\lambda} \int_{-1}^{1} \frac{1}{2} \frac{|\text{Im}(a)|^{1-s}}{| \omega_x - a|^{2-s}} \, d \omega_x
    \end{align*}
    where $a=- \frac{\hat{G}_{z,\alpha}(x,x)^{-1}}{\lambda}$.

    From Lemma \ref{lemma:7.8} it reduces to:
    \begin{equation*}
    \mathbb{E}_x [I] \le (|\zeta|^s + 1)\, C_s(\rho)\, |G_{E+i\eta}^{(\zeta)}(x,y)|^s
    \end{equation*}
    ...
    with $Z = \int_{-1}^{1} \frac{1}{2} (|\zeta|^s + 1)^{-1} \, d\zeta$. This definition is valid since:
    \begin{enumerate}
    ...
    and
    \begin{align*}
    &\int_{\mathbb{R}} \frac{C_s(\rho)}{Z} (|\zeta|^s + 1) (|\zeta|^s + 1)^{-1} \cdot \frac{1}{2} \chi_{[-1,1]} |G_{E+i\eta}^{(\zeta)}(x,y)|^s \, d\zeta \\
    &= \frac{C_s(\rho)}{Z} \int_{-1}^{1} \frac{1}{2} |G_{E+i\eta}^{(\zeta)}(x,y)|^s \, d\zeta \\
    &= \frac{C_s(\rho)}{Z} \mathbb{E}_x \left[ |G_{E+i\eta}^{(\zeta)}(x,y)|^s \right]
    \end{align*}

    Averaging over all $(w_n)_{n \in \mathbb{Z}^d}$ means
    \begin{equation*}
    \mathbb{E}[I] \le C_1 \mathbb{E} \left[ |G_{E+i\eta}^{(\zeta)}(x,y)|^s \right]
    \end{equation*}
    ...
    \begin{equation*}
        \left( \int_\Sigma \mathbb{E}[|\mathrm{Im}(G_{E + i\eta,\alpha})(x,y)|] \, dE \right)^2 \le (C_1)^2 \left( \int_\Sigma \mathbb{E}[|G_{E+i\eta,\alpha}(x,y)|^s] \, dE \right)^2.
    \end{equation*}
    ...
    \begin{equation}
        \mathbb{E}[Q(x,y;\Sigma)] \le C_1 \liminf_{|\eta| \downarrow 0} \int_\Sigma \mathbb{E}[|G_{E+i\eta,\alpha}(x,y)|^s] \, dE.
    \end{equation}
    This concludes the proof.
\end{proof}

\begin{proof}[Proof of Proposition \ref{prop:almostloc}]
    As we mentioned in Definition \ref{def:Q}, for bounded sets $\Sigma \subset \mathbb{R}$, 
    $$Q(x,y,\Sigma) \coloneqq \sup_{\substack{g \in C_c(\Sigma) \\ |g| \le 1}} | g(H_{\omega,\alpha})(x,y)|.$$ 
    For $g \in C_c(\Sigma)$ and $\xi \in \mathbb{R}$,
    \begin{equation*}
    g(\xi) = \lim_{\eta \downarrow 0} \frac{\eta}{\pi} \int_\Sigma \frac{g(E)}{(\xi - E)^2 + \eta^2} \, dE.
    \end{equation*}
    By the spectral theorem, this translates to the operator matrix elements as
    \begin{equation*}
        g(H_{\omega,\alpha})(x,y) = \lim_{\eta \downarrow 0} \frac{1}{\pi} \int_\Sigma g(E) \, \mathrm{Im}(G_{E + i\eta, \alpha})(x,y) \, dE.
    \end{equation*}
    Taking the absolute value, passing it inside the integral, and applying the supremum bound $|g(E)| \le 1$, we obtain:
    \begin{equation}
        Q(x,y, \Sigma) \le \liminf_{\eta \downarrow 0} \frac{1}{\pi} \int_{\Sigma} |\mathrm{Im}(G_{E + i\eta,\alpha})(x,y)| \, dE.
    \end{equation}
    Averaging a positive function (see the proof of the \textit{a priori bound}, Lemma \ref{lemma:aprioribound}), and therefore applying Fatou's and Fubini's theorems, gives
    \begin{equation} \label{eq:7.29}
        \mathbb{F}[Q(x,y;I) ]\le \liminf_{\eta \downarrow 0} \frac{1}{\pi} \int_I \mathbb{E}[|\mathrm{Im}(G_{E + i\eta,\alpha})(x,y)|] \, dE.
    \end{equation}
    By the Cauchy-Schwarz inequality,
    ...
    \end{equation} 
    where $\mathrm{Im}(G_{E + i\eta,\alpha})(x,y) >0$ when $\eta>0$.\\
    Splitting the integrand in \eqref{eq:7.29} into powers of $(1-s)$ and $s$, and applying \eqref{eq:7.30} to just the $s$ power, we obtain
    \begin{equation} \label{eq:7.31}
        \begin{split}
        &\left( \int_I \mathbb{E}[|\mathrm{Im}(G_{E + i\eta,\alpha})(x,y)|] \, dE \right)^2 \\
        &= \left( \int_I \mathbb{E}\left[ |\mathrm{Im}(G_{E + i\eta,\alpha})(x,y)|^{1-s} |\mathrm{Im}(G_{E + i\eta,\alpha})(x,y)|^s \right] \, dE \right)^2 \\
    ...
    \end{equation}
    where we again applied the Cauchy-Schwarz inequality.
    Since we are working under translation invariance, we will now focus on bounding one of the integrals, knowing that the same holds for the other.

    Using the subadditivity property $\left|\sum_{n} a_{n}\right|^{s} \le \sum_{n} |a_{n}|^{s}$, we estimate
    \begin{equation} \label{eq:7.32}
        |\mathrm{Im}(G_{E + i\eta,\alpha})(x,y)|^s \le \frac{1}{2^s} \left( |G_{E+i\eta,\alpha}(x,y)|^s + |G_{E-i\eta,\alpha}(x,y)|^s \right).
    \end{equation}
    Since $|\mathrm{Im}(G_{E + i\eta,\alpha})(x,x)|=|\mathrm{Im}(G_{E - i\eta,\alpha})(x,x)|$, the results that we obtain for the first term are the same as those for the second, and we can finally bound 
    \begin{equation}
        I \coloneqq|\mathrm{Im}(G_{E + i\eta,\alpha})(x,x)|^{1-s}|G_{E+i\eta,\alpha}(x,y)|^s. \label{eq:3.2.15}
    \end{equation}
    
    Now, we consider Proposition \ref{prop:useofresolventidentity} and note that 
    \[
        \frac{G_z^{(\nu)}(x,y)}{G_z^{(\nu)}(x,x)} = \frac{\frac{G_{z,\alpha}(x,y)}{1 + \nu G_{z,\alpha}(x,x)}}{\frac{G_{z,\alpha}(x,x)}{1 + \nu G_{z,\alpha}(x,x)}} =\frac{G_{z,\alpha}(x,y)}{G_{z,\alpha}(x,x)}.
    \]
    For $\nu = \zeta -\lambda\omega_x$, we obtain the operator $H_\zeta \coloneqq H_{\omega,\alpha} + (\zeta - \lambda\omega_x)\chi_{\{x\}}$.\\
    In particular, we recall from the \textit{a priori bound} (Lemma \ref{lemma:aprioribound}) that 
    $$\hat{H}_{\omega,\alpha} \coloneqq H - \lambda\omega_x\,\chi_{\{x\}} \qquad \text{ and }\qquad G_{z,\alpha}(x,x) =\frac{1}{{\hat{G}_{z,\alpha}(x,x)}^{-1} + \lambda\omega_x}.$$
    This implies for \eqref{eq:3.2.15} that
    \begin{align*}
        &|\mathrm{Im}(G_{z,\alpha}(x,x))|^{1-s}|G_{z,\alpha}(x,y)|^s \\
        &= \left| \mathrm{Im}\left(\frac{1}{{\hat{G}_{z,\alpha}(x,x)}^{-1} + \lambda\omega_x}\right)\right|^{1-s}\left|\frac{1}{{\hat{G}_{z,\alpha}(x,x)}^{-1} + \lambda\omega_x} \frac{G_z^{(\zeta)}(x,y)}{G_z^{(\zeta)}(x,x)}\right|^s\\
    ...
    where we used $ |\sum _n a_n|^s \le \sum_n |a_n|^s$ in the last step.\\
    Now, we average $I$ over the random potential $v = \lambda\omega_x$, which has the density $\rho_V(v) = \frac{1}{2\lambda} \chi_{[-\lambda,\lambda]}(v)$, in order to obtain the following bound,
    \begin{align*}
        \mathbb{E}_x [I] &\le \mathbb{E}_x [L] \\
        &= (|\zeta|^s + |a|^s) \cdot |G_{E+i\eta}^{(\zeta)}(x,y)|^s \cdot \int_{-\lambda}^{\lambda} \frac{|\text{Im}(a)|^{1-s}}{| v - a|^{2-s}} \rho_V(v) \, d v \\
        &= (|\zeta|^s + |a|^s) \cdot |G_{E+i\eta}^{(\zeta)}(x,y)|^s \cdot \int_{-\lambda}^{\lambda} \frac{|\text{Im}(a)|^{1-s}}{| v - a|^{2-2s}} \frac{\rho_V(v)}{|v-a|^s} \, d v
    \end{align*}
    with $a = -{\hat{G}_{z,\alpha}(x,x)}^{-1}$.
    From Lemma \ref{lemma:7.8}, noting that $(|\zeta|^s + |a|^s) \le (|\zeta|^s + 1) \max\{1, |a|^s\}$, this rigorously reduces to
    \begin{equation*}
        \mathbb{E}_x [I] \le (|\zeta|^s + 1)\, C_s(\rho_V)\, |G_{E+i\eta}^{(\zeta)}(x,y)|^s
    \end{equation*}
    where $C_s(\rho_V)$ implicitly contains the disorder parameter $\lambda$.
    ...
    with $Z = \int_{-1}^{1} \frac{1}{2} (|\zeta|^s + 1)^{-1} \, d\zeta$ to normalize. This definition of desnity function is valid since
    \begin{enumerate}
        \item[(a)] $\rho_\zeta$ is non-negative, since we consider $|\zeta|$.
        \item[(b)] $\rho_\zeta$ is continuous and $\int_{\mathbb{R}} \rho_\zeta \, d\zeta = 1$.
    \end{enumerate}
    ...
    and
    \begin{align*}
    &\int_{\mathbb{R}} \frac{C_s(\rho_V)}{Z} (|\zeta|^s + 1) (|\zeta|^s + 1)^{-1} \cdot \frac{1}{2} \chi_{[-1,1]} |G_{E+i\eta}^{(\zeta)}(x,y)|^s \, d\zeta \\
    &= \frac{C_s(\rho_V)}{Z} \int_{-1}^{1} \frac{1}{2} |G_{E+i\eta}^{(\zeta)}(x,y)|^s \, d\zeta \\
    &= \frac{C_s(\rho_V)}{Z} \mathbb{E}_x \left[ |G_{E+i\eta}^{(\zeta)}(x,y)|^s \right]
    \end{align*}
...
\begin{remark}
    For Proposition \ref{prop:almostloc},we use $\rho_V(v) = \frac{1}{2\lambda} \chi_{[-\lambda, \lambda]}(v)$. Since
    \[
    \sup_{v \in \mathbb{R}} (1+|v|^s)\rho_V(v) = \frac{1+\lambda^s}{2\lambda} < \infty.
    \]
    we can bound with $ C_s(\rho_V)$, which implicitly depends on the disorder parameter $\lambda$.
\end{remark}

\clearpage
\chapter{SAW representation}\label{sec:SAW}
In this chapter, we are going to use the SAW representation method in order to prove Theorem \ref{thm:theorem1.1} and Theorem \ref{thm:theorem1.2}. First, we will give the main definitions regarding self-avoiding walks. Second, we develop a new representation for the Green function using SAWs and finally use it to prove the two theorems.
\section{SAW Definitions}\label{sec:SAW.1}
All the definitions reguarding SAW, are generated by the fractional moments of the discrete Laplacian. We find them in \cite{madras2013self} and the generalization for the fractional Anderson model in \cite{disertori2024}.
...
Recall Lemma \ref{lemma:summability}. Thus, for $s \in \left(\frac{d}{d+2\alpha},1\right)$, $D(x,y)$ generates a transition probability $$p(x \to y) \coloneq \frac{D(x,y)}{\sum_{w \ne x} D(x,w)}$$ for $x \ne y$ in $\mathbb{Z}^d$, and is translation-invariant. 
...
\begin{figure}[ht!]
    ...
    \caption{self-avoiding walks from $x$ to $y$ with near-neighbors jumps and long range jumps}
\end{figure}
...
\begin{definition}[Susceptibility]
    ...
    \end{equation*}
    The series converges uniformly and absolutely if and only if $\beta < R_{\chi^D}$, where $R_{\chi^D}$ is critical radius of convergence. It is defined as
        \[
...
\begin{proof}
Let $c_N$ denote the number of self-avoiding walks (SAWs) of length $N$ starting from the origin $0 \in \mathbb{Z}^d$ for $\alpha=1$, and let $S_N(0, x)$ denote the set of such walks ending at $x \in \mathbb{Z}^d$. By definition:
\[
...
The radius of convergence $R'$ of the original series $\sum_{N=0}^{\infty} c_N|\beta|^N$ is
\[
...
The proofs in this Section follow the ones given by Schenker in \cite{schenker2015}. We are going to work with the Green function defined on a volume. Therefore, for any $\Lambda \subseteq \mathbb{Z}^d$ (finite or infinite), the Green function changes to:
...
The first SAW representation is given byt the following Claim.
\vspace{.2cm}
...
The kinetic operator $(-\Delta)^\alpha$ could connect all arbitrarily distant sites in the lattice, not just nearest neighbors. Therefore, $\left( H_\omega^\Lambda - (H_\omega^{\{x\}} \oplus H_\omega^{\Lambda \setminus \{x\}}) \right)$ isolates all off-diagonal jumps originating from or terminating at $x$:
\[
H_\omega^\Lambda - (H_\omega^{\{x\}} \oplus H_\omega^{\Lambda \setminus \{x\}}) = \sum_{w \in \Lambda \setminus \{x\}} (-\Delta)^\alpha(x, w) \left( \langle \delta_w, \cdot \rangle \delta_x + \langle \delta_x, \cdot \rangle \delta_w \right) =: T_{x,w}.
\]

Now, considering the matrix elements for $(x, y)$ with $x \neq y$ and applying matrix multiplication:
\[
 (H_\omega^\Lambda - z)^{-1}(x,y) - 0 = -\sum_{x_1, x_2 \in \Lambda}(H_\omega^\Lambda - z)^{-1} (x,x_1) T_{x_1, x_2} (H_\omega^{\{x\}} \oplus H_\omega^{\Lambda \setminus \{x\}} - z)^{-1}(x_2,y).
\]
...
\end{theorem}
\begin{proof}
Now we separate the case in which $x_1 = y$ (note that for the fractional operator, jumps can be arbitrarily long, so we no longer require $|x-y|=1$).
\[
...
The final results reguarding the decay of the fractional moments of the green function, depends on the estimated decay of the two point function. In fact we can prove the following Propositions.
...
\[
c_N^D\;\le\; K_\varepsilon (\mu_d+\varepsilon)^N \qquad \text{for all } N \ge 0.
\]
